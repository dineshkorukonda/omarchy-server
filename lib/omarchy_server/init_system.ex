defmodule OmarchyServer.InitSystem do
  @moduledoc """
  ...
  @table_name :omarchy_server_init_systems
  ...
  def detect(session, server_id) do
    runner = fn cmd ->
      if Code.ensure_loaded?(OmarchyServer.SSH) and
           function_exported?(OmarchyServer.SSH, :exec, 2) do
        apply(OmarchyServer.SSH, :exec, [session, cmd])
      else
